...
def load_data_into_db():
    ...
    try:
        conn = sqlite3.connect(constants.DB_PATH + constants.DB_FILE_NAME)
        df = load_data(f'{constants.DATA_DIRECTORY}leadscoring_inference.csv')
...
def map_city_tier():
    ...
    try:
        ...
        df_lead_scoring["city_tier"] = df_lead_scoring["city_mapped"].map(city_tier_mapping.city_tier_mapping)
...
def map_categorical_vars():
    ...
    try:
        ...
        new_df = df_lead_scoring[~df_lead_scoring['first_platform_c'].isin(significant_categorical_level.list_platform)] # get rows for levels which are not present in list_platform
        new_df['first_platform_c'] = "others" # replace the value of these levels to others
        old_df = df_lead_scoring[df_lead_scoring['first_platform_c'].isin(significant_categorical_level.list_platform)] # get rows for levels which are present in list_platform
        df = pd.concat([new_df, old_df]) # concatenate new_df and old_df to get the final dataframe
        
        # all the levels below 90 percentage are assgined to a single level called others
        new_df = df[~df['first_utm_medium_c'].isin(significant_categorical_level.list_medium)] # get rows for levels which are not present in list_medium
        new_df['first_utm_medium_c'] = "others" # replace the value of these levels to others
        old_df = df[df['first_utm_medium_c'].isin(significant_categorical_level.list_medium)] # get rows for levels which are present in list_medium
        df = pd.concat([new_df, old_df]) # concatenate new_df and old_df to get the final dataframe
        
        # all the levels below 90 percentage are assgined to a single level called others
        new_df = df[~df['first_utm_source_c'].isin(significant_categorical_level.list_source)] # get rows for levels which are not present in list_source
        new_df['first_utm_source_c'] = "others" # replace the value of these levels to others
        old_df = df[df['first_utm_source_c'].isin(significant_categorical_level.list_source)] # get rows for levels which are present in list_source
        df = pd.concat([new_df, old_df]) # concatenate new_df and old_df to get the final dataframe
...
def interactions_mapping():
    ...
    try:
        ...
        df_event_mapping = pd.read_csv( f'{constants.INTERACTION_MAPPING}interaction_mapping.csv',index_col=[0])
        
        id_vars = constants.INDEX_COLUMNS_TRAINING
        ...
        df_model_op = df_pivot[constants.INDEX_COLUMNS_TRAINING]
        df_model_op = df_model_op.drop(columns=constants.NOT_FEATURES, axis=1)
