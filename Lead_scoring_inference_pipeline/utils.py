...
def read_input_data_from_db(db_path, db_file_name, table_name):
    cnx = sqlite3.connect(db_path + db_file_name)
    df = pd.read_sql('select * from '+ table_name, cnx)
    df.drop(columns=['level_0', 'index'], axis = 1, inplace=True, errors='ignore')
    cnx.close()
    print("Data has been extracted successfully from lead_scoring_model_experimentation for table"+table_name)
    return df

def save_data_to_db(db_path, db_file_name, input_data, table):
    cnx = sqlite3.connect(db_path + db_file_name)
...
def encode_features():
    ...
    SAMPLE USAGE
        encode_features()
    '''
    df = read_input_data_from_db(constants.DB_PATH, constants.DB_FILE_NAME,'interactions_mapped')
    ...
    encoded_df.drop(columns=['app_complete_flag'],inplace=True,axis=1,errors='ignore')
    save_data_to_db(constants.DB_PATH, constants.DB_FILE_NAME,encoded_df,'features')
    return 'Features are encoded and saved in features table in db'
...
def get_models_prediction():
    ...
    SAMPLE USAGE
        load_model()
    '''
    X=read_input_data_from_db(constants.DB_PATH, constants.DB_FILE_NAME, 'features')
    ...
    save_data_to_db(constants.DB_PATH, constants.DB_FILE_NAME, predicted_output, 'predicted_output')
...
def prediction_ratio_check():
    ...
    '''
    input_data = read_input_data_from_db(constants.DB_PATH, constants.DB_FILE_NAME, 'predicted_output')
...
def input_features_check():
    ...
    SAMPLE USAGE
        input_col_check()
    '''
    input_data = read_input_data_from_db(constants.DB_PATH, constants.DB_FILE_NAME, 'features')
    source_cols = input_data.columns.to_list()
    print(source_cols,'->source columns')
    print(constants.ONE_HOT_ENCODED_FEATURES,'->One hot encoded features')
    if source_cols == constants.ONE_HOT_ENCODED_FEATURES:
